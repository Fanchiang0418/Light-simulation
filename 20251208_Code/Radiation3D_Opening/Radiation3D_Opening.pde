...
void drawLights3D() {
  ...
  float groundY = stageBottom;       // 燈條腳底在這個 Y
  ...
  // 如果想整圈一起旋轉，可以加一個偏移：
  float ringRotation = 0;          // 例如改成 HALF_PI 會整圈轉 90°

  for (int i = 0; i < numLights; i++) {
    // 0~(numLights-1) 均勻映射到 240° 的弧形
