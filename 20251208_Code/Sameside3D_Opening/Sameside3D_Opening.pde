...
// 一圈在地板上的正圓 + 直立燈條
// 一圈在地板上的正圓 + 每一根彎成括號形的燈條（3D 貝茲曲線）
// 一圈在地板上的正圓 + 每一根是「完整圓形」的燈條
// 一圈在地板上的正圓 + 每一根是「開口都朝右邊」的弧形燈條
// 一圈在地板上的正圓 + 每一根是「開口都朝右邊」的弧形燈條
// ＋ 只佔 240°，留一個 120° 的缺口
// 一圈在地板上的 240° 弧形燈座 + 每一根是弧形燈條
// 開口方向在地面 XZ 平面「從 +X 轉 90 度」
void drawLights3D() {
  pushMatrix();
  noStroke();
  ...
  float stageTop    = 80;
  float stageBottom = height - 180;   // 燈條底部高度（地板 Y）
  ...
  float groundY = stageBottom;       // 燈條腳底 Y
  ...
  for (int i = 0; i < numLights; i++) {
    ...
    // 直線燈條的下端 / 上端（高度跟原本一樣）
    float x0 = baseX;
    float y0 = groundY - segmentHeight/2.0;                    // 下端
    float z0 = baseZ;
